package com.blogforum.notebook.web.exception;
...
public class MyCustomExceptionResolver implements HandlerExceptionResolver {
	private static Logger logger = Logger.getLogger(MyCustomExceptionResolver.class);
		...
		if (!(request.getHeader("accept").indexOf("application/json") > -1 || (request
			...
			try {
				...
				if (exception instanceof BusinessException) {
					map.put("status", ((BusinessException) exception).getCode());
					map.put("msg", exception.getMessage());
				}else if (exception instanceof NoteBusinessException) {
					map.put("status", ((NoteBusinessException) exception).getCode());
					map.put("msg", exception.getMessage());
				}else {
					map.put("status", BizErrorEnum.SYS_EXCEPTION.getCode());
					map.put("msg", BizErrorEnum.SYS_EXCEPTION.getMsg());
